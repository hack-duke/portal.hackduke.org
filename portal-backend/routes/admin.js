...
router.post('/applications', async (req, res) => {
  ...
  try {
    const users = await User.find({}).select('name email school major graduationYear applications');
    res.json(users);
  } catch (error) {
...
router.put('/applications/:applicationId/status', async (req, res) => {
  ...
  try {
    const user = await User.findOne({ 'applications._id': req.params.applicationId });
    if (!user) {
